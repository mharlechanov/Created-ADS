...
function findDealerByZip(data,zip){
    ...
    if (elem2 == 0 ) {
        ...
        for (i = 0; i < x.length; i++) {
            ...
            if (zipCode.substring(0, 8) == userZip.substring(0, 8)) {
                elems[j] = i;j++;	
    ...
    if (elem2 == 0 ) {
        ...
        for (i = 0; i < x.length; i++) {
            ...
            userZip = userZip.toString();
            //userZip = userZip.toLowerCase();
            if (zipCode.substring(0, 7) == userZip.substring(0, 7)) {
                elems[j] = i;j++;	
                elem2 = elems[Math.floor(Math.random() * j)];
    ...
    if (elem2 == 0 ) {
        ...
        for (i = 0; i < x.length; i++) {
            ...
            if (zipCode.substring(0, 6) == userZip.substring(0, 6)) {
                elems[j] = i;j++;	
                elem2 = elems[Math.floor(Math.random() * j)];
            }
        }
...
function onZipDataLoaded(shopData){
    shopURL = "https://www.google.com/maps/search/?api=1&query="  + encodeURIComponent(shopData["zip"] + "," + shopData["Address"]);
    console.log(shopURL);
};
...
loadDealerJson("https://media.permodo.net/ads/PC21_00463/v3/data/data.json");
